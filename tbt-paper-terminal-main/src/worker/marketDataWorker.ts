...
const BINANCE_WS_URLS = [
  'wss://testnet.binance.vision',   // 优先使用测试网
  'wss://stream.binance.com:9443',  // 官方主要端点
  'wss://stream.binance.com:443',   // 备用端口
];
let currentWsUrlIndex = 0;
let wsBaseOverride: string | null = null;
let wsUrlPool: string[] = [...BINANCE_WS_URLS];
...
let restFallbackStarter: ReturnType<typeof setTimeout> | null = null;
...
function applyWsBaseOverride(wsBase?: string): void {
  if (!wsBase) {
    wsBaseOverride = null;
    wsUrlPool = [...BINANCE_WS_URLS];
    return;
  }
  const cleaned = wsBase.replace(/\/+$/, '');
  wsBaseOverride = cleaned;
  wsUrlPool = [cleaned, ...BINANCE_WS_URLS.filter((u) => u !== cleaned)];
  currentWsUrlIndex = 0;
}

async function fetchTicker24h(symbol: string): Promise<void> {
  if (!orderBookManager) return;
  const url = `${BINANCE_REST_URL}/ticker/24hr?symbol=${symbol.toUpperCase()}`;
  try {
    ...
    orderBookManager.updateTicker24h({
      h: String(data.highPrice ?? '0'),
      l: String(data.lowPrice ?? '0'),
      v: String(data.volume ?? '0'),
      p: String(data.priceChange ?? '0'),
      P: String(data.priceChangePercent ?? '0'),
    });
  } catch {
    // ignore
  }
...
async function fetchRecentTrades(symbol: string): Promise<void> {
  const url = `${BINANCE_REST_URL}/trades?symbol=${symbol.toUpperCase()}&limit=50`;
  try {
    const response = await fetch(url);
    if (!response.ok) return;
    const data = await response.json();
    ...
    const fresh = data
      .filter((t) => typeof t.id === 'number' && t.id > lastRestTradeId)
      .map((t) => ({
        id: String(t.id),
        symbol: symbol.toUpperCase(),
        price: String(t.price),
        quantity: String(t.qty),
        quoteQty: String(t.quoteQty ?? (parseFloat(t.price) * parseFloat(t.qty))),
        time: Number(t.time),
    ...
    if (fresh.length > 0) {
      lastRestTradeId = Math.max(lastRestTradeId, ...fresh.map((t) => Number(t.id)));
      ...
      sendMessage<TradeUpdatePayload>('TRADE_UPDATE', { trades: fresh });
    }
  } catch {
    // ignore
  }
...
function startRestFallback(symbol: string): void {
  ...
  const poll = async () => {
    ...
    await fetchRecentTrades(symbol);
...
function stopRestFallback(): void {
  if (restFallbackStarter) {
    clearTimeout(restFallbackStarter);
    restFallbackStarter = null;
  }
...
async function connect(symbol: string): Promise<void> {
  ...
  restFallbackStarter = setTimeout(() => {
  ...
  pendingConnectTimer = setTimeout(() => {
    ...
    const baseUrl = wsUrlPool[currentWsUrlIndex % wsUrlPool.length];
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        stopHeartbeat();
        if (currentSymbol) {
          startRestFallback(currentSymbol);
        }
...
function scheduleReconnect(): void {
  ...
  if (reconnectAttempts % 3 === 0) {
    currentWsUrlIndex = (currentWsUrlIndex + 1) % wsUrlPool.length;
...
self.onmessage = (event: MessageEvent<WorkerMessage>) => {
  ...
  switch (type) {
    case 'SUBSCRIBE': {
      const { symbol, wsBase } = payload as SubscribePayload;
      applyWsBaseOverride(wsBase);
      connect(symbol);
      break;
    }
    case 'UNSUBSCRIBE': {
